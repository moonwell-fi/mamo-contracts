...
contract RewardsDistributorSafeModuleIntegrationTest is BaseTest {
    ...
    uint256 public constant MAMO_REWARD_AMOUNT = 0; // No MAMO rewards
    uint256 public constant VIRTUALS_REWARD_AMOUNT = 1e18;
    ...
    function setUp() public override {
        ...
        virtualsToken = IERC20(addresses.getAddress("VIRTUALS"));
        ...
        if (addresses.isAddressSet("REWARDS_DISTRIBUTOR_MAMO_VIRTUALS")) {
            module = RewardsDistributorSafeModule(addresses.getAddress("REWARDS_DISTRIBUTOR_MAMO_VIRTUALS"));
        } else {
            DeployRewardsDistributorSafeModule deployer = new DeployRewardsDistributorSafeModule();
            module = RewardsDistributorSafeModule(deployer.deploy(addresses));
            deployer.validate(addresses);
        }
    ...
    function test_addRewardsFirstTime() public {
        // Only deal VIRTUALS tokens, no MAMO
        ...
        emit RewardAdded(MAMO_REWARD_AMOUNT, VIRTUALS_REWARD_AMOUNT, notifyAfter);
        ...
        assertEq(amountToken1Stored, MAMO_REWARD_AMOUNT, "Mamo amount should be 0");
        assertEq(amountToken2Stored, VIRTUALS_REWARD_AMOUNT, "VIRTUALS amount should be the same");
    ...
    function test_addRewardsSecondTimeFailIfNotNotified() public {
        ...
        assertEq(amountToken2Stored, VIRTUALS_REWARD_AMOUNT);
    ...
    function test_addRewardsSecondTimeSuccess() public {
        ...
        deal(address(virtualsToken), address(safe), 1);
        // No MAMO tokens needed
    ...
    function test_notifyRewards() public {
        ...
        vm.expectEmit(true, true, true, true);
        emit RewardsNotified(MAMO_REWARD_AMOUNT, VIRTUALS_REWARD_AMOUNT, block.timestamp);
