...
| Review remediation — `nav()` prices the held reward balance | `LeveragedAeroValuation.netEquityUsdc` gained a `heldReward` term: the gauge reward token already claimed into the strategy (every `_unwindLiquidity`'s `gauge.withdraw` auto-claims one), priced on `Layout.aeroUsdFeed` — the same feed the sale floor uses and the same field a venue migration rewrites, threaded through two new `Config` members (`gauge`, `rewardFeed`). Previously unpriced, so a deposit placed between an unwind and the next `compound()` captured a pro-rata slice of the harvest step. **HELD BALANCE ONLY** — `gauge.earned()` is deliberately not added (it reverts `"NA"` for an unstaked tokenId, which would give `nav()` a revert path that bricks deposits/redeems). The feed read is guarded on `balance > 0`, so steady state adds no oracle dependency; with a balance held, a stale reward feed **fail-closes `nav()`** | *this change* |
