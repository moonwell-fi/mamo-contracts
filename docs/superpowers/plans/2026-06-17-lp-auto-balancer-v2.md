...
- [ ] **Step 1: Bring the shared LP plumbing from the PR #54 branch, then copy V1 as the base**

This branch is cut from `main`, which has **none** of the Aerodrome plumbing (no interfaces, libs, mocks, or V1 contract — all of it landed on `feat/lp-auto-balancer`). Pull the neutral, proven plumbing + the V1 contract (as the copy base) from that branch:

```bash
git checkout feat/lp-auto-balancer -- \
  src/interfaces/ICLGauge.sol src/interfaces/ICLPool.sol src/interfaces/ICLPositionManager.sol \
  src/interfaces/INonfungiblePositionManager.sol src/interfaces/IQuoter.sol src/interfaces/IPriceFeed.sol \
  src/libraries/uniswap/ \
  test/mocks/MockCLGauge.sol test/mocks/MockCLPool.sol test/mocks/MockPositionManager.sol \
  test/mocks/MockPriceFeed.sol test/MockERC20.sol \
  src/LPAutoBalancer.sol
cp src/LPAutoBalancer.sol src/LPAutoBalancerV2.sol
forge build   # pull any further missing dependency the compiler names from the same branch
```
Do **not** keep `src/LPAutoBalancer.sol` (V1) committed on this branch unless needed — it is only the copy source for `LPAutoBalancerV2.sol`; delete it after copying if it isn't referenced. (`ISwapRouter` is intentionally not pulled — V2 has no swap.)
