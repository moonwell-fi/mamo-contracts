...
- **Size budget.** Balancer adds: 3 state vars + begin/finish (mostly reusing `_exitAll` / mint internals) + a 3-line passthrough + one setter. Must stay < 24,576 bytes (currently 23,986; if it overflows, move begin/finish valuation snapshots into `LPBalancerLib`).
...
**R3 — backlog (Med-low).** No Base sequencer-uptime feed check in `LPBalancerLib.readFeed`, and floor valuation tolerates `maxOracleDelay` = 26h staleness — a frozen-but-fresh-looking feed makes the floor ≈ no-op (same stale price on both sides), while a > 26h-stale feed wedges rebuild via `StaleOracle`. Compensating control: the backend refuses to `unwindForSwap` on feeds older than ~2× heartbeat and observes a grace window after sequencer recovery (backend spec §7). On-chain fix (sequencer-uptime feed + heartbeat-scaled floor staleness) queued for the next contract rev.
