...
- **Size budget.** Balancer adds: 3 state vars + begin/finish (mostly reusing `_exitAll` / mint internals) + a 3-line passthrough + one setter. Must stay < 24,576 bytes. Currently 23,955 (621 B headroom) after the 2026-07-10 review pass moved `validateGauge` and `validatePoolAndNft` into `LPBalancerLib` — the escape valve this bullet predicted. Re-run `forge build --sizes` before trusting this figure; it drifts with every edit.
