...
## 7. Open items — the launch-now critical path

Posture decision 2026-08-05: **launch now.** The market gap (idle stock tokens, no credible manager) is open today, the competitive field is measured-empty, and the seat is worth a deliberate year-one cost center — this launch is bought for stance and the option, not revenue, and no document should call it break-even. Target ~2–3 months: contracts (2–4 weeks), the P0 keeper set, and the audit run in parallel; legal review runs at the org level and keys the final date — engineering does not pace on it. **Boosted USDG stays dark** until its measured triggers fire (`BOOSTED_USDG_SIZING.md` §6): launching a ~4–5% product beside a subsidized 7% is worse than not launching it; it flips on at the subsidy cliff as the upsell to the basket user base. Launch posture: genesis-capped, staker-priority.
...
Not blocking launch, but on the clock: the gas subsidy expires around **2026-09-29**, after which per-user rebalance economics need re-checking against the fee model in §2 — the fee base at launch caps is ~$3.7k/yr, so gas netting is not academic. **Bridge MAMO to 4663 via Wormhole pre-launch** [infra — ready] so capacity gating is native on day one.

**The flywheel at launch scale, designed for what it is.** The Drop is not the launch story at $1.1M of caps (~$70/week reads as parody — accrue and distribute quarterly until the number earns a weekly cadence). The story is **capacity gating on measured scarcity**: reserve ~50% of each basket's cap for MAMO stakers, and convert the monitoring tripwires (equity-DEX depth, feed coverage, subsidy runway, loop spread) into **cap-raise and product-add triggers with staker priority** — caps rise only when re-measured depth supports them, and stakers keep first access every time they do. Honest scarcity, mechanical utility now; revenue share arrives when depth growth or the subsidy cliff converts stance into cashflow.
