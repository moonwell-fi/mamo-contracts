...
The two issues below are therefore permanent properties of the deployed registry
(`0x46a5624C2ba92c08aBA4B206297052EDf14baa92`) and are mitigated operationally, not in code.
See Sherlock 2026-07 findings #41 and #42.

**1. An explicit `strategyTypeId` can squat a free slot.**
...
implementation is stranded permanently — `addStrategy` requires an exact match against the latest,
so every factory pinned to it stops working, and its existing proxies would accept only an
unrelated implementation as their upgrade target. Neither is reversible:
`latestImplementationById` is written in exactly one place, and the
"Implementation already whitelisted" guard blocks re-registering the displaced implementation.
...
- **Create a new type with an explicit id `>= 5`**, after asserting the slot is free
  (`latestImplementationById(id) == address(0)`) and the counter has not reached it
...
**2. `getBackendAddress()` has no stable identity.**
It returns `getRoleMember(BACKEND_ROLE, 0)`. BACKEND_ROLE is shared by the strategy operator and
every factory, and OpenZeppelin's `EnumerableSet` removes by swap-and-pop, so revoking the member
at index 0 moves the last member — possibly a factory — into that slot. Strategies compare
`msg.sender` against this value directly. Operating rule: during any key rotation, verify
`getBackendAddress()` before and after, and grant the incoming operator before revoking the
outgoing one rather than the reverse.

A future registry deployment should fix both in code (advance the counter past an explicit id, or
reject ids that do not already exist; and store the operator in a dedicated admin-settable slot).
That is a migration project, out of scope here.
