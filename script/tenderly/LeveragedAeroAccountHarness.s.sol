 ...
 *      the 012 multisig proposal calls. It exists because the proposal's `LEVERAGED_AERO_STRATEGY`
 *      / `LEVERAGED_AERO_VAULT` address-book keys are deliberately NOT committed to
 ...
 *      code lives at those addresses on real Base). Instead this harness injects them at RUNTIME via
 ...
 *        LEVERAGED_AERO_STRATEGY  - the vendored LeveragedAerodromeCLStrategy clone
 *        LEVERAGED_AERO_VAULT     - the LeveragedAeroVault (share ERC-20)
 */
contract LeveragedAeroAccountHarness is Script {
    ...
    /// @notice Build the FPS address book and inject the vnet-only pooled-layer keys from env.
    function _addresses() internal returns (Addresses addresses) {
        uint256[] memory chainIds = new uint256[](1);
        chainIds[0] = block.chainid;
        addresses = new Addresses("./addresses", chainIds);

        _addFromEnv(addresses, "LEVERAGED_AERO_STRATEGY");
        _addFromEnv(addresses, "LEVERAGED_AERO_VAULT");
    }
